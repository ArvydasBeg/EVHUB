  ...
  buyButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (currency === "BNB") {
        ...
        const decimals = 18; // Jei tavo USDC ant BSC turi 6 decimals, pakeisk į 6!
        const amountInWei = ethers.utils.parseUnits(
          amount.toString(),
          decimals
        );
        tx = await usdc.transfer(recipientAddress, amountInWei);
        await tx.wait();
        txHash = tx.hash;
      } else {
      ...
      // SIUNČIAM IR txHash, currency, value!
      await fetch(`${API_URL}/buy`, {
        ...
        body: JSON.stringify({
          ...
          txHash,
          currency,
          value: amount, // kiek BNB ar USDC išsiuntė
        }),
      });
  ...
  async function updateAirdropStats(wallet) {
    ...
    if (data.claimed && data.claimInfo) {
      ...
      <div>
        ...
        Supercharge bonus: <span>${data.claimInfo.presaleBonus.toLocaleString()}</span> EVHUB<br>
    ...
    <span class="presale-bonus">+25,000 with supercharge</span><br>
    Invite friends: <span>${
      hasPresale ? "1,000" : "100"
    } EVHUB per friend</span><br>
    Supercharge: <span class="yes">${hasPresale ? "YES" : "NO"}</span><br>
